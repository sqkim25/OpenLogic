...
\label{PRIM-SEM001}
\index{structure}\index{domain}\index{interpretation}
...
\emph{standard model of arithmetic},\index{standard model of arithmetic} because it interprets the
...
A structure is \emph{covered}\index{covered structure} if every element of the domain is the
...
\label{PRIM-SEM004}
\index{variable assignment}\index{assignment}
...
\label{PRIM-SEM010}
\index{entailment}\index{logical consequence}
...
\label{DEF-SEM002}
\index{satisfiability}\index{satisfiable}
...
\label{DEF-SEM007}
\index{definability}\index{definable set}
...
\label{rem:R}
  ...
  the Isomorphism Lemma (\cref{THM-SEM001}) fails in general:
...
\label{DEF-SEM017}
\index{standard model of arithmetic}\index{model!standard arithmetic}
...
(\cref{DEF-SEM008}). But structures can be very different and still
...
\begin{defn}
An \emph{automorphism}\index{automorphism} of a structure $\Struct{M}$ is an isomorphism
of $\Struct{M}$ onto itself.
\end{defn}

\begin{defn}[Elementary Substructure] % DEF-SEM011
\label{DEF-SEM011}
\index{elementary substructure}\index{substructure!elementary}
...
A structure for $\Lang{L_A}$ is \emph{standard}\index{model!standard} if it is
...
A structure~$\Struct{M}$ for $\Lang{L_A}$ is \emph{non-standard}\index{model!non-standard}
...
Cantor's theorem (\cref{thm:cantorQ}), any two such orderings are isomorphic. It follows
...
\label{THM-SEM002}
\index{coincidence lemma}\index{extensionality}
...
\begin{proof}
Follows from the Coincidence Lemma by \cref{cor:sat-sentence}.
\end{proof}
