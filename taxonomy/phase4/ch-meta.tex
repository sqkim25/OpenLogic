...
\index{complete consistent set}
A set~$\Gamma$ of sentences is \emph{complete} iff for any
sentence~$!A$, either $!A \in \Gamma$ or $\lnot !A \in \Gamma$.
\end{defn}

\begin{ex}\label{ex:complete-set}
Let $\Struct{M}$ be any structure for~$\Lang{L}$.  The set
$\Gamma = \{!A : \Sat{M}{!A}\}$ of all sentences true in~$\Struct{M}$
is complete: for any sentence~$!A$, either $\Sat{M}{!A}$ (so $!A \in
\Gamma$) or $\Sat{M}{\lnot !A}$ (so $\lnot !A \in \Gamma$).  It is
also consistent, since $\Struct{M} \models \Gamma$.
\end{ex}
...
\index{maximally consistent set}
...
\begin{ex}\label{ex:mcs}
The set $\Gamma = \{!A : \Sat{M}{!A}\}$ from
\cref{ex:complete-set} is maximally consistent: it is
consistent (since $\Struct{M} \models \Gamma$), and adding any
$!B \notin \Gamma$ produces an inconsistent set, because $!B \notin
\Gamma$ means $\Sat{M}{\lnot !B}$, so $\lnot !B \in \Gamma$, and
$\Gamma \cup \{!B\} \Proves \lfalse$.
\end{ex}

Every maximally consistent set is complete and consistent; it therefore
has all the properties established in \cref{prop:ccs}.
...
\begin{ex}\label{ex:saturated-set}
Let $\Lang{L}$ contain a constant~$c$ and a one-place
predicate~$P$.  If a consistent set~$\Gamma$ contains
$\lexists[x][\Atom{P}{x}]$ and also
$\lexists[x][\Atom{P}{x}] \lif \Atom{P}{c}$, then $\Gamma$~has a
``witness'' for its existential: the constant~$c$ tells us
\emph{which} element satisfies~$P$.  A saturated set provides such
witnesses for \emph{every} existential sentence it contains.
\end{ex}

The following definition will be used in the proof of the next lemma.

\begin{defn} \label{defn:henkin-exp}
\index{Henkin expansion}
Let $\Lang L'$ be as in \cref{prop:lang-exp}.  Fix an enumeration
...
constants to form~$\Lang{L'}$.  By \cref{prop:lang-exp}, $\Gamma$
is still consistent in the richer language.  Further, let $!D_i$ be as
in \cref{defn:henkin-exp}.  Let
\begin{align*}
\Gamma_0 & = \Gamma \\
...
\cref{defn:henkin-exp}), $c_n$ does not occur in~$!A_n(x_n)$ nor
in~$\Gamma_n$.
...
\item First suppose that $\lexists[x][!A(x)]
      ...
      \cref{prop:ccs}\ref{prop:ccs-if}),
      and \cref{prop:ccs}\ref{prop:ccs-prov-in}, $!A(c)
      \in \Gamma$.
    ...
    \cref{prop:ccs}\ref{prop:ccs-prov-in},
...
\begin{ex}\label{ex:term-model}
Suppose $\Lang{L}$ contains a constant~$c$, a one-place function
symbol~$f$, and a two-place predicate~$R$.  The closed terms
of~$\Lang{L}$ are $c$, $f(c)$, $f(f(c))$, \dots.  In the term
model~$\Struct{M}(\Gamma^*)$, the domain is $\{c, f(c), f(f(c)),
\ldots\}$; the interpretation of~$f$ maps each term~$t$ to the
term~$f(t)$; and $\tuple{s, t} \in \Assign{R}{M(\Gamma^*)}$ iff
$\Atom{R}{s, t} \in \Gamma^*$.
\end{ex}

\begin{lem}
\label{lem:val-in-termmodel} Let $\Struct M(\Gamma^*)$ be the term model
of \cref{defn:termmodel}; then $\Value{t}{M(\Gamma^*)} = t$.
\end{lem}
...
Let $\Struct M(\Gamma^*)$ be the term model of \cref{defn:termmodel}.
\begin{enumerate}
...
\item $!A \ident !B \land !C$:
    ...
    \cref{prop:ccs}\ref{prop:ccs-and}, this is the case
...
\item $!A \ident \lforall[x][!B(x)]$:
    $\Sat{M(\Gamma^*)}{\lforall[x][!B(x)]}$ iff
      ...
      (\cref{prop:quant-termmodel}).  By induction hypothesis, this
      is the case iff $!B(t) \in \Gamma^*$ for all terms~$t$; by
      \cref{prop:saturated-instances}, this in turn is the case
...
\item $!A \ident \lexists[x][!B(x)]$:
    $\Sat{M(\Gamma^*)}{\lexists[x][!B(x)]}$ iff
      ...
      By \cref{prop:saturated-instances}, this in turn is the
...
\index{factored term model}
\label{defn:term-model-factor}
Let $\Struct M = \Struct M(\Gamma^*)$ be the term model
for~$\Gamma^*$ from \cref{defn:termmodel}.  Then $\Struct{\equivclass{M}{\approx}}$ is the following
structure:
\begin{enumerate}
...
\equivrep{t}{\approx}$.  \cref{prop:approx-equiv}
...
\begin{proof}
Follows from \cref{prop:approx-equiv} by induction on~$n$.
\end{proof}
...
\begin{proof}
The proof is similar to that of \cref{lem:val-in-termmodel}.
\end{proof}
...
\begin{proof}
By induction on~$!A$, just as in the proof of \cref{lem:truth}.
...
Suppose $\Gamma$ is consistent. By
  \cref{lem:henkin}, there is a saturated consistent set
  $\Gamma' \supseteq \Gamma$. By \cref{THM-DED005} (Lindenbaum's Lemma), there
...
$\Gamma$ is satisfiable.
  ...
  then by \cref{lem:truth-factored}, for all sentences~$!A$,
...
\begin{proof}
Note that the $\Gamma$'s in \cref{cor:completeness} and
\cref{CP-002} are universally quantified.  To make sure we
do not confuse ourselves, let us restate \cref{CP-002}
...
\cref{CP-002} gives us that $\Gamma \cup \{\lnot !A\}$ is
inconsistent.  By
...
(\cref{cor:satisfiable-consistent}, from \S\ref{META.1}),
...
\begin{ex}[De Bruijn--Erd\H{o}s]\label{ex:compactness-app}
Every graph in which every finite subgraph is $k$-colorable is itself
$k$-colorable.  To see this, let $\Lang{L}$ have a constant for each
vertex and $k$ unary predicates $C_1, \ldots, C_k$.  For each
vertex~$v$, include $C_1(v) \lor \cdots \lor C_k(v)$; for each
edge~$\{u, v\}$ and color~$i$, include $\lnot(C_i(u) \land C_i(v))$.
Every finite subset of this set mentions finitely many vertices, hence
is satisfiable by hypothesis.  By the Compactness Theorem, the whole
set is satisfiable---giving a $k$-coloring.
\end{ex}
...
\begin{rem}[Skolem's paradox]\label{rem:skolem-paradox}
If ZFC is consistent, the L\"owenheim--Skolem theorem gives it an
enumerable model~$\Struct{M}$.  Yet ZFC proves ``there exist
uncountable sets.''  The resolution: $\Struct{M}$ thinks some set is
uncountable because it lacks a witnessing bijection---the bijection
exists outside~$\Struct{M}$ but not inside it.  ``Uncountable'' is
not absolute but depends on the ambient model (Skolem, 1922).
\end{rem}
...
\begin{ex}\label{ex:bounded-quantifier}
The formula $\bforall{x < \num{3}}{\Atom{P}{x}}$ abbreviates
$\lforall[x][(x < \num{3} \lif \Atom{P}{x})]$.  In the standard
model~$\Struct{N}$, this says that $P$ holds of $0$, $1$, and~$2$.
It is a $\Delta_0$~formula since all quantifiers are bounded.
\end{ex}
...
Suppose $!A \ident t_1 = t_2$. By \cref{lem:q-proves-clterm-id},
...
such that $n + k + 1 = m$. By \cref{lem:q-proves-clterm-id},
$\Th{Q} \Proves \eq[t_1][\num n]$ and $\Th{Q} \Proves \eq[t_2][\num m]$,
...
The base case is given by \cref{lem:atomic-completeness},
...
!A(\num 0) \land \dots \land !A(\num{k-1})$.
By \cref{lem:bounded-quant-equiv} it follows that
...
\cref{lem:atomic-completeness}, $\Th{Q} \Proves \lnot !A$.
...
$\Sat{N}{!A(\num n)}$. But $!A(\num n)$ is a
$\Delta_0$ formula, so by \cref{lem:delta0-completeness}
...
$!G_\Th{T}$ by \cref{lem:cons-G-unprov}. Since there is no
...
\begin{proof}
  ...
  \Proves/ !G_\Th{T}$ by \cref{lem:cons-G-unprov}.  By
  \cref{lem:omega-cons-G-unref}, $\Th{T} \Proves/ \lnot !G_\Th{T}$.
...
\cref{CP-008} below). This computability-theoretic argument
...
\emph{Proof idea.}  Define a ``Rosser provability predicate''
$\ORProv[\Th{T}](y)$ asserting ``$y$ has a proof shorter than any
refutation of~$y$.''  Apply the fixed-point lemma to obtain a
sentence~$!R$ asserting its own non-Rosser-provability.  Consistency
alone (without $\omega$-consistency) suffices to show $\Th{T}
\Proves/ !R$ and $\Th{T} \Proves/ \lnot !R$.
...
\begin{equation}
  ...
  \label{RT}
\end{equation}
In contrast to the proof of \cref{CP-005},
...
\index{Godel's second incompleteness theorem}
\label{CP-006}
...
of \cref{CP-005}. We have shown ``If $\Th{PA}$
...
(\cref{DEF-DED014}). If $\Th{T}$ derives $\OProv[\Th{T}](\gn{!A}) \lif !A$,
...
\index{definability in N@definability in $\Struct{N}$}
...
(\cref{THM-DED006}), there is a formula
...
\index{undecidability of Q@undecidability of $\Th{Q}$}
\label{CP-008}
...
\emph{cannot} express---limits on decidability, completeness, and
definability of specific theories.  We now turn to what first-order
logic \emph{can} guarantee about the structure of valid implications:
Craig's interpolation theorem and Beth's definability theorem reveal a
strong modularity property of the entailment relation.
...
\begin{ex}\label{ex:separation}
Let $\Gamma = \{\Atom{P}{c}\}$ and $\Delta = \{\lnot \Atom{P}{c}\}$,
where $P$ and $c$ occur in both languages.  Then $!C = \Atom{P}{c}$
separates $\Gamma$ and~$\Delta$: $\Gamma \Entails \Atom{P}{c}$
(trivially) and $\Delta \Entails \lnot \Atom{P}{c}$ (trivially).  In
general, finding an interpolant whose vocabulary is common to both
sides is much harder.
\end{ex}
...
compactness (\cref{CP-003}), there are \emph{finite} subsets $\Gamma_0$ of $\Gamma$
...
\emph{Proof idea.}  We prove the contrapositive: if $\{!A\}$ and
$\{\lnot !B\}$ are inseparable (no sentence in their common
vocabulary entails one and refutes the other), then $!A \land \lnot
!B$ is satisfiable, so $\not\Entails !A \lif !B$.  The key step
uses Lindenbaum's lemma to extend both sets to maximally consistent
sets that agree on the shared vocabulary, then constructs a single
structure satisfying both.
...
\item\label{part-b} $\Gamma_{n+1}$ and $\Delta_n$ are inseparable in
    $\Lang{L}'_0$.
\end{enumerate}
The basis for \ref{part-a} is given by \cref{lem:sep1}. For
...
\item It remains to consider the case where $!A_0$ is existential, so
  ...
  $\Delta_0$ are inseparable, so that by \cref{lem:sep2} also
...
existential, by \cref{lem:sep2}); if $\Delta_{n+1} = \Delta_n$
...
by \cref{lem:sep2}); and if  $\Gamma_{n+2} = \Gamma_{n+1}$ then
...
(\cref{CP-002}). Similarly, $\Delta^*$ has a
...
The distinction matters: explicit definability gives a concrete
formula~$!C$ equivalent to~$P$; implicit definability merely
guarantees uniqueness---any two predicates satisfying the same axioms
must be identical.  Beth's theorem (below) shows that, remarkably,
these two notions coincide in first-order logic.
...
\]
By compactness (\cref{CP-003}), there are finite sets $\Delta_0 \subseteq \Sigma(P)$
...
\]
By Craig's Interpolation Theorem (\cref{CP-011}) there is a sentence $!C(c_1,\dots, c_n)$
...
\cref{prop:qr-finite}: given $a \in \Domain{M}$, let
...
isomorphic structures are isomorphic (\cref{thm:p-isom1}),
...
Recall \cref{prop:qr-finite}: there are only finitely many
...
\emph{Proof idea.}  Given an $L$-sentence~$!E$ over a finite
relational language, use the back-and-forth characterization of
elementary equivalence (\cref{thm:b-n-f}) and a compactness
argument to find~$n$ such that $!E$ cannot distinguish structures
agreeing on all first-order sentences of quantifier rank~$\le n$.
Then $!E$ is equivalent to a Boolean combination of such sentences,
showing $\tuple{L, \models_L} \le \tuple{F, \models}$.

\begin{proof}
By \cref{lem:lindstrom}, it suffices to show that for any $!E
...
language, by \cref{thm:b-n-f} we can replace the statement
...
proof of \cref{thm:abstract-p-isom}, let $\Struct{M}^*$ be the
...
\Struct{N_{n^*}}$. But by \cref{thm:abstract-p-isom},
...
(\cref{CP-001}), for each system $S$ we have: if $\Gamma
\Proves_S !A$ then $\Gamma \Entails !A$.  By the Completeness Theorem
(\cref{CP-002}), for each system $S$: if $\Gamma \Entails !A$
