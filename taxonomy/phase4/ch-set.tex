...
In \cref{ch:bst} we developed set-theoretic foundations
...
Unlike the naive sets of \cref{ch:bst}, formal sets are objects
...
$\emptyset$ exists (by \cref{AX-SET002}). Now for any
set~$x$, the set $x \cup \{x\}$ exists by
\cref{AX-SET003:consequences}. So, applying this a few times, we
...
\cref{ch:bst} can be made rigorous within~$\Zminus$.
\end{rem}
...
\]
By \cref{SET.3:lemordsegments}, $f$ preserves order. One shows
...
\cref{SET.3:woalwayscomparable} gives rise to a thought: we could
...
in the definition of $\omega$ (\cref{SET.2:defnomega}). This
...
\cref{SET.3:wo:strictorder}. Hence $x \in c \in b$ gives $x
...
$\phi$, and this is an ordinal by \cref{SET.3:ordmemberord}.
\end{proof}
...
(\cref{DEF-SET005}) twice. Say that $x$ is \emph{comparable}
...
ordinal by \cref{SET.3:ordmemberord}. By the first induction
...
\cref{SET.3:ordmemberord}, so $\alpha \in O$. So $O$ is
transitive, and hence $O$ is an ordinal by
\cref{SET.3:corordtransitiveord}. Hence $O \in O$,
contradicting irreflexivity (\cref{SET.3:wo:strictorder}).
\end{proof}

The Burali-Forti paradox shows that the ordinals form a \emph{proper
class} (see \cref{PRIM-SET003}). Ordinals are sets which are
...
\index{ordinal representation theorem}
Every well-ordering is isomorphic to a unique ordinal.
\end{thm}

\begin{proof}
Let $\tuple{A, <}$ be a well-ordering. By
\cref{SET.3:ordtrichotomy} and
\cref{SET.3:wellordnotinitial}, it is isomorphic to at most one
...
\cref{SET.3:wellordnotinitial}. For the second, let $\ordtype{A,
...
ordinal by \cref{SET.3:corordtransitiveord}. And $\alpha \in
...
hence an ordinal by \cref{SET.3:corordtransitiveord}.
...
(\cref{SET.3:simpletransrecursion}) on ordinals $\delta \leq
\alpha$.
...
\begin{proof}
For each $\alpha$, by \cref{SET.3:transrecursionfun} there is a
...
\cref{SET.3:transrecursionfun}.
\end{proof}

Note that \cref{DEF-SET006} is a \emph{schema}. Crucially, we
cannot expect $\sigma$ to define a function (i.e., a set), since then
$\dom{\sigma}$ would be the set of all ordinals, contradicting
Burali-Forti (\cref{SET.3:buraliforti}).
...
\[
	...
	\end{cases}
\]
By \cref{DEF-SET006}, there is a term $\sigma(x)$ such that
...
(\cref{SET.3:simpletransrecursion}) confirms:
...
\cref{SET.3:thmOrdinalRepresentation}, form
$\alpha = \Setabs{\ordtype{B, R}}{\tuple{B, R} \in C}$.
By \cref{SET.3:corordtransitiveord}, $\alpha$ is an ordinal
...
between them (see \cref{ch:bst}). Just as we introduced ordinals
...
There is a snag with \cref{DEF-SET007}: we would like
...
\label{AX-SET009}
\index{axiom!well-ordering}
Every set can be well-ordered.
\end{axiom}

This guarantee is unavailable in $\ZF$ alone. The Well-Ordering axiom
is the final axiom of $\ZFC$ (see \cref{SET.2:zfc}). Its
...
$\tuple{A, R}$. By \cref{SET.3:thmOrdinalRepresentation},
...
\cref{SET.4:CardinalsExist}, there are bijections $\card{A} \to A$
...
\cref{SET.4:CardinalsBehaveRight} guarantees Cantor's Principle:
...
uses Replacement and Well-Ordering; the proof in \cref{ch:bst}
works in $\Zminus$.)
\end{rem}
...
\cref{ch:bst}) gives $\cardless{\cardfont{a}}{\Pow{\cardfont{a}}}$.
By \cref{SET.4:CardinalsExist}, $\cardfont{a} <
\card{\Pow{\cardfont{a}}}$.
\end{proof}
...
\label{DEF-SET012}
\index{cumulative hierarchy}
\index{von Neumann hierarchy}
\begin{align*}
...
(\cref{SET.3:simplerecursionschema}): take $A = \emptyset$,
...
Regularity (\cref{SET.2:zfentailsregularity}).
...
\index{epsilon-induction@$\in$-induction}
For any formula $\phi$:
\[
...
$\setrank{x} \in \setrank{A}$ by \cref{SET.5:rankmemberslower},
...
\cref{SET.3:thmOrdinalRepresentation}.

\begin{defn}[Ordinal Addition] \label{SET.5:defordplus}
\index{ordinal arithmetic!addition}
...
\index{ordinal arithmetic!multiplication}
...
\index{ordinal arithmetic!exponentiation}
...
\begin{proof}
From Cantor's Theorem (THM-BST001, \cref{ch:bst}) and
\cref{SET.5:SizePowerset2Exp}.
\end{proof}
...
\index{cardinal arithmetic!simplification}
...
$\cardfont{a}$ (\cref{SET.4:NoLargestCardinal}), and Transfinite
...
by \cref{SET.4:omegaisacardinal}; successors are cardinals by
definition; limits are cardinals by
\cref{SET.4:unioncardinalscardinal}.
...
\cref{SET.5:continuumis2aleph0} tells us $\card{\Real} = \beth_1$.
...
\omega + \omega$ by \cref{SET.5:ordsetrankalpha}). So $\Z$
does not prove the existence of $\omega + \omega$.
\end{proof}

This is why \cref{SET.3:thmOrdinalRepresentation} cannot be
...
$\kappa$ is a cardinal by \cref{SET.4:unioncardinalscardinal},
...
\begin{proof}
As in \cref{SET.5:alephfixed}, using ``$\beth$'' in place of
``$\aleph$''.
\end{proof}
...
\begin{proof}
Let $\kappa$ be a $\beth$-fixed point (\cref{SET.5:bethfixed}).
Then $\card{V_\kappa} = \beth_\kappa = \kappa$ by
\cref{SET.5:stagesize}.
\end{proof}
...
Transfinite Recursion (\cref{DEF-SET006}), define:
\begin{align*}
	g(0) &= f(A)\\
	g(\alpha) &=
		\begin{cases}
...
(\cref{SET.3:HartogsLemma}) there is an ordinal $\beta$ with
...
(\cref{DEF-SET005}), Trichotomy
(\cref{SET.3:ordtrichotomy}), Burali-Forti
(\cref{SET.3:buraliforti}), and Ordinal Representation
(\cref{SET.3:thmOrdinalRepresentation}). The machinery of
transfinite recursion (Theorems~\ref{DEF-SET006}
and~\ref{SET.3:simplerecursionschema}) enabled the definition of the
cumulative hierarchy $V_\alpha$ (\cref{DEF-SET012}) and
ordinal arithmetic. Adding the Well-Ordering axiom (AX-SET009,
\S\ref{AX-SET009}) yielded $\ZFC$ and a robust theory of cardinals.
Cardinal arithmetic
(\cref{SET.5:cardplustimesmax}) simplifies dramatically for
infinite cardinals; but the Continuum Hypothesis
(\cref{DEF-SET015}) remains independent of $\ZFC$. The
equivalence of Well-Ordering, Choice, and Zorn's Lemma
(\cref{THM-SET001}) unifies the theory.
\end{rem}
