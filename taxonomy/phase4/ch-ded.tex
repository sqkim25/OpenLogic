...
derivation with no undischarged assumptions is a \emph{proof}\index{proof} (of a
theorem).
...
Quantifiers interact with derivability through a generalization
principle:
...
\index{Hilbert system}
\index{axiomatic system}
\label{DEF-DED005}
...
\index{modus ponens}
\index{rule!modus ponens}
\label{AX-DED001}
If $!B$ and $!B \lif !A$ already occur in a derivation, then $!A$ is
a correct inference step. We abbreviate this rule as~$\MP$.
\end{defn}

\begin{ex}\label{DED2-ex:lif-identity}
Here is an axiomatic derivation of $\Proves p \lif p$:
\begin{derivation}
1. & $p \lif ((p \lif p) \lif p)$ & \eqref{ax:lif1}\\
2. & $(p \lif ((p \lif p) \lif p)) \lif
     ((p \lif (p \lif p)) \lif (p \lif p))$ & \eqref{ax:lif2}\\
3. & $(p \lif (p \lif p)) \lif (p \lif p)$ & $\MP$, 1, 2\\
4. & $p \lif (p \lif p)$ & \eqref{ax:lif1}\\
5. & $p \lif p$ & $\MP$, 4, 3
\end{derivation}
Even this simple identity requires five lines, illustrating why
axiomatic systems are easy to reason \emph{about} but hard to use
directly.
\end{ex}
...
\index{soundness!axiomatic system}
...
rules come in pairs---an introduction\index{introduction rule} and an elimination\index{elimination rule} rule for each
...
\begin{ex}\label{DED3-ex:lif-identity}
As a first example, here is a derivation of $p \lif p$ from no
undischarged assumptions:
\begin{prooftree}
\AxiomC{$\Discharge{p}{1}$}
\DischargeRule{\Intro{\lif}}{1}
\UnaryInfC{$p \lif p$}
\end{prooftree}
The sole leaf is the assumption~$p$, discharged by $\Intro{\lif}$ at
label~$1$.  Since no assumptions remain open, this is a derivation
of $p \lif p$ from~$\emptyset$.
\end{ex}
...
\index{soundness!natural deduction}
...
two forms: a left rule\index{rule!left} (introducing the operator in the antecedent)
and a right rule\index{rule!right} (introducing it in the succedent).
...
\begin{ex}\label{DED4-ex:lif-identity}
A derivation of the sequent $\Sequent p \lif p$:
\begin{center}
\Axiom$p \fCenter p$
\RightLabel{\RightR{\lif}}
...
\DisplayProof
\end{center}
The topmost sequent $p \Sequent p$ is an initial sequent.  The
$\RightR{\lif}$ rule moves~$p$ from the antecedent into the
conditional on the succedent side.
\end{ex}
...
\index{soundness!sequent calculus}
...
The cases fall into three groups: structural rules (weakening,
contraction, exchange, cut), propositional rules ($\lnot$, $\land$,
$\lor$, $\lif$), and quantifier rules ($\lforall$, $\lexists$).  In
each case the argument shows that validity is preserved from
...
\begin{ex}\label{DED5-ex:lif-identity}
To show that $p \lif p$ is a tautology, we construct a tableau
starting from $\sFmla{\False}{p \lif p}$:
\begin{center}
\begin{tabular}{rl}
1. & $\sFmla{\False}{p \lif p}$ \\
2. & $\sFmla{\True}{p}$ \hfill ($\TRule{\False}{\lif}$, 1)\\
3. & $\sFmla{\False}{p}$ \hfill ($\TRule{\False}{\lif}$, 1)\\
   & $\otimes$ \hfill (2, 3)
\end{tabular}
\end{center}
The $\TRule{\False}{\lif}$ rule applied to line~1 produces both
$\sFmla{\True}{p}$ and $\sFmla{\False}{p}$ on the same branch.
Since $p$ appears with both signs, the branch closes~($\otimes$).
Every branch being closed, the tableau is closed, confirming
that $p \lif p$ is valid.
\end{ex}
...
\item Every signed formula not among the assumptions results from a
  ...
  branch above it.
\end{enumerate}
A branch\index{branch!tableau} is \emph{closed}\index{branch!closed} iff it contains both $\sFmla{\True}{!A}$
and~$\sFmla{\False}{!A}$, and \emph{open}\index{branch!open} otherwise. A tableau with
...
The proof systems of DED.2--DED.5 provide the deductive machinery;
...
\index{theory!Robinson arithmetic}
\label{DEF-DED011}
...
\index{omega-consistency@$\omega$-consistency}
\label{DEF-DED013}
...
The proof systems and formal theories of the preceding sections
support several deep results that bridge deduction and metatheory.
...
\index{Lob's theorem@L\"ob's theorem}
...
\emph{Proof idea.}  Apply the fixed-point lemma to the formula
$\OProv[\Th{T}](y) \lif !A$, obtaining a sentence~$!D$ with $\Th{T}
\Proves !D \liff (\OProv[\Th{T}](\gn{!D}) \lif !A)$.  Informally,
$!D$ says ``if I am provable, then $!A$.''  The derivability
conditions P1--P3 then show $\Th{T} \Proves !D$, whence $\Th{T}
\Proves !A$.
