...
DEF-META001, Soundness, \S META.1 and THM-META002, Completeness, \S
META.3), that these notions coincide.
...
of the completeness theorem (see THM-META002, Completeness, \S META.3).
...
deduction theorem (see THM-DED001, \S DED.7), $\Gamma \Proves \lnot
...
the content of the cut-elimination theorem, see THM-META004, Cut
Elimination, \S META.4), but in a system with cut it is a primitive
...
\S META.3) produces a conservative extension of the original theory.
\end{rem}
...
consistent set (by Lindenbaum's Lemma, see THM-DED005, \S DED.7).
...
The proofs of these facts depend on the particular proof system.  In
axiomatic deduction, they follow from the logical axioms and modus
ponens together with the deduction theorem. In natural deduction, they
follow directly from the introduction and elimination rules. In the
sequent calculus, they follow from the left and right logical rules.
...
\begin{proof}
(1) From axiom~\eqref{ax:lnot1} we derive $\{\lnot !A\} \Proves !A
...
PRIM-SYN009, Language, \S SYN.1 for the general notion of a
...
\S CMP.4), which is the key hypothesis of the incompleteness theorems.
...
of arithmetic~$\Struct{N}$ (see DEF-SEM012, Standard Model, \S SEM.5).
...
THM-BST002, Rosser's Theorem, \S BST.3).
...
Theorem (see THM-DED007, \S DED.7) and the Second Incompleteness
Theorem (see THM-BST003, \S BST.4).
...
conditions P1--P3 (see DEF-DED014, \S DED.6). If $\Th{T}$ derives
...
incompleteness theorem (see THM-BST003, Second Incompleteness Theorem,
\S BST.4).  Take $!A \defis \lfalse$.  If $\Th{T} \Proves
