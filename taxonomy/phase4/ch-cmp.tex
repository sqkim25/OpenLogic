...
\label{DEF-CMP001}\index{partial function}\index{function!partial}
...
\label{DEF-CMP002}\index{total function}\index{function!total}
...
\label{DEF-CMP-COMP}\index{composition}
...
Together with the projection functions\index{projection function}\index{function!projection} $\Proj{n}{i}(x_0, \dots,
...
\label{PRIM-CMP002}\index{primitive recursion}
...
\label{PRIM-CMP002-SET}\index{primitive recursive function}\index{function!primitive recursive}
The set of \emph{primitive recursive functions} is the set of
functions from $\Nat^n$ to $\Nat$, defined inductively by the
following clauses:
\begin{enumerate}
\item $\Zero$\index{zero function}\index{function!zero} is primitive recursive.
\item $\Succ$\index{successor function}\index{function!successor} is primitive recursive.
...
\label{DEF-CMP003}\index{characteristic function}\index{function!characteristic}
...
\label{PRIM-CMP003}\index{unbounded search}\index{minimization}\index{mu-operator@$\mu$-operator}
...
\label{PRIM-CMP001}\index{partial recursive function}\index{function!partial recursive}
...
\label{DEF-CMP002-REC}\index{recursive function}\index{function!recursive}\index{general recursive function}
...
\label{PRIM-CMP004}\index{Turing machine}
...
\label{DEF-CMP-TMCOMP}\index{computable function}\index{function!computable}
...
\label{DEF-CMP-TMPCOMP}\index{partial computable function}\index{function!partial computable}
...
\label{PRIM-CMP005}\index{Church-Turing thesis}
...
\label{PRIM-CMP006}\index{computable set}\index{decidable set}\index{decidability}
...
\label{PRIM-CMP007}\index{computably enumerable set}\index{recursively enumerable set}
...
element of~$S$.  By Kleene's normal form theorem (DEF-CMP005,
\S\ref{CMP.4} below),
\[
...
\label{DEF-CMP010b}\index{computably enumerable set}
...
the halting problem (see \S\ref{CMP.4}).
\end{proof}
...
establish fundamental limits on what can be computed.  Several results
in \S\ref{CMP.3} (e.g., the non-computability of~$K_0$) were stated
using the halting problem; we now give the detailed proofs.  We use
diagonalization to prove that the halting problem is unsolvable, and
that there is no universal computable function for the total computable
functions.
...
assigning codes to their definitions; see PRIM-CMP011, \S\ref{CMP.5}).  The
...
\label{PRIM-CMP008}\index{halting function}
...
\label{PRIM-CMP008-PROB}\index{halting problem}\index{undecidability}
...
\label{THM-CMP002}\index{halting problem}
...
total computable $\fn{Un}'(e,n)$ agreeing with $\fn{Un}$ wherever the latter
is defined (by returning $0$ when $h(e,n)=0$), contradicting
\cref{PRIM-CMP010}.  An alternative, more direct proof proceeds by
diagonalization: define $g(n)$ to equal $0$ when $h(n,n)=0$ and be
...
\label{THM-CMP004}
\label{DEF-CMP-NF}\index{Kleene normal form}
...
\label{PRIM-CMP012}\index{universal Turing machine}\index{Turing machine!universal}
...
\label{THM-CMP004-SMN}\index{s-m-n theorem@$s$-$m$-$n$ theorem}
...
\label{DEF-CMP009-REP}\index{representable function}\index{function!representable}
...
\label{DEF-CMP-REPREL}\index{representable relation}
...
\label{THM-CMP-REPCOMP}\index{representability theorem}
...
the equality relation $=$) are representable, and that representable functions are
...
\times x_1)]$, and the equality relation $=$ by
...
\label{DEF-CMP007}\index{productive set}
...
\label{PRIM-CMP009}\index{many-one reduction}\index{reduction}
...
\label{DEF-CMP008}\index{complete c.e. set}\index{computably enumerable set!complete}
...
A further variant of the halting set is $K_1 = \Setabs{e}{\cfind{e}(0)
\fdefined}$, which records those indices whose program halts on
input~$0$.
...
\label{DEF-CMP011}\index{axiomatizable theory}
...
\label{DEF-CMP014}\index{computable inseparability}
...
\label{THM-CMP003}\index{Rice's theorem}
...
\label{THM-CMP005}\index{recursion theorem}\index{fixed-point theorem}
